...
namespace vae::gen {
    ...
    namespace {
        ...
        std::string MainSource(const Options& options) {
            return "// Generated by VAE. The app: build the document, run it.\n"
                 ...
                 + (options.script.empty()
                        ? std::string{}
                        : "        // Beside the binary, not beside the working directory: this app\n"
                          "        // has to run from wherever it is double-clicked.\n"
                          "        layer->SetScript(FileSystem::ExecutableDir() / \""
                              + options.script + "\");\n")
                 + "        layer->Start();\n"
                   "\n"
                   "        AppSpec spec;\n"
                   ...
                   "\n"
                   "        // Fonts, pictures and translations are picked up from beside the\n"
                   "        // binary by the engine itself — see FontDB::LoadDefaults and\n"
                   "        // RunLayer::Start. Nothing here has to name them.\n"
                   "\n"
        ...
        std::string PremakeSource(const Options& options, const std::filesystem::path& sdk) {
            return "-- Generated by VAE. An ordinary premake project with no VAE-specific magic in\n"
                   "-- it beyond one include: sdk/vae.lua knows the engine's link line so this file\n"
                   "-- does not have to, and does not go stale when the engine gains a dependency.\n"
                   "--\n"
                   "--   premake5 gmake && make                 -- Dist, the shipping build\n"
                   "--   premake5 gmake && make config=debug    -- with symbols\n"
                   "--\n"
                   "-- The binary is written beside this file, together with the fonts, pictures,\n"
                   "-- translations and script it needs. That whole folder is the app: copy it to\n"
                   "-- another machine and it runs, with no VAE installed there.\n"
                   "\n"
                   "include \"" + sdk.generic_string() + "/vae.lua\"\n"
                   "\n"
                   "workspace \"" + options.appName + "\"\n"
                   "   -- Dist first, so a bare `make` produces the build you would ship.\n"
                   "   configurations { \"Dist\", \"Debug\", \"Release\" }\n"
                   "   architecture \"x86_64\"\n"
    ...
    bool EmitProject(const doc::Document& document, const std::filesystem::path& directory,
        ...
        // No SDK, no buildable project. Emitting one that points at a directory with no vae.lua
        // in it would hand somebody a folder that fails at their `make`, with the reason two
        // machines away.
        const std::filesystem::path sdk = FileSystem::SdkRoot();
        if (sdk.empty()) {
            if (error) *error = "this VAE has no SDK beside it (looked for "
                              + (FileSystem::EngineRoot() / "sdk" / "vae.lua").string()
                              + ") — reinstall with ./install.sh, or set VAE_SDK";
            return false;
        }
        if (!FileSystem::WriteText(directory / "premake5.lua", PremakeSource(options, sdk))) {
